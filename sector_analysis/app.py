...
CACHE_DIR = ROOT / "ema_scanner" / "data" / "_sector_cache"   # precomputed snapshot
...
# Scopes: which membership file to read. All three are subsets of the Nifty 500
# sector map, so sectors resolve for every symbol.
SCOPES = [
    {"key": "nifty50",  "label": "Nifty 50",  "file": "nifty50.csv"},
    {"key": "sensex30", "label": "Sensex 30", "file": "sensex30.csv"},
    {"key": "nifty500", "label": "Nifty 500", "file": "nifty500_industry.csv"},
]
SCOPE_KEYS = {s["key"] for s in SCOPES}

# Symbols that have candle data but are absent from the current niftyindices
# sector map (e.g. post-demerger tickers). Small manual bridge so heavyweights
# are not dropped. Survives a re-fetch of the reference csvs.
SECTOR_OVERRIDES = {
...
app = FastAPI(title="Sector Analysis")
...
# ─────────────────────────────── reference data ────────────────────────────
def _read_reference():
    """Return (industry_by_sym, company_by_sym, scope_syms:{scope->[sym,...]})."""
    ind: dict[str, str] = {}
    comp: dict[str, str] = {}
    n500_path = REF_DIR / "nifty500_industry.csv"
    with n500_path.open(encoding="utf-8") as f:
        for r in csv.DictReader(f):
            s = (r.get("Symbol") or "").strip()
            if not s:
                continue
            ind[s] = (r.get("Industry") or "").strip() or "Unclassified"
            comp[s] = (r.get("Company") or "").strip()
    for s, sec in SECTOR_OVERRIDES.items():
        ind.setdefault(s, sec)

    scope_syms: dict[str, list[str]] = {}
    for sc in SCOPES:
        p = REF_DIR / sc["file"]
        syms: list[str] = []
        with p.open(encoding="utf-8") as f:
            for r in csv.DictReader(f):
                ...
                if s:
                    syms.append(s)
        scope_syms[sc["key"]] = syms
    return ind, comp, scope_syms
...
def _build() -> dict:
    ind, comp, scope_syms = _read_reference()
    union = set()
    for syms in scope_syms.values():
        union |= set(syms)
    close_wide = _load_close(union)
    ...
    return {
        "ind": ind, "comp": comp, "scope_syms": scope_syms,
    ...
    with _LOCK:
        ...
        if cw_p.exists() and rt_p.exists() and sig_p.exists() and sig_p.read_text().strip() == sig:
            try:
                ...
                ind, comp, scope_syms = _read_reference()
                as_of = close_wide.index.max().strftime("%Y-%m-%d") if not close_wide.empty else None
                data = {"ind": ind, "comp": comp, "scope_syms": scope_syms,
                        "close": close_wide, "rets": rets, "as_of": as_of}
            except Exception:
                data = None
        if data is None:
...
def _scope_members(snap: dict, scope: str, rets: pd.DataFrame) -> list[str]:
    """Symbols in scope that have both a sector and price data (as of `rets`)."""
    ind = snap["ind"]
    have = set(rets.index) if not rets.empty else set()
    out = []
    for s in snap["scope_syms"].get(scope, []):
        if s in have and s in ind:
            out.append(s)
    return out
...
def _sector_groups(snap: dict, scope: str, rets: pd.DataFrame) -> dict[str, list[str]]:
    ...
    for s in _scope_members(snap, scope, rets):
        groups[ind[s]].append(s)
    return groups
...
def _sector_rows(snap: dict, scope: str, rets: pd.DataFrame) -> tuple[list[dict], dict]:
    ...
    total_in_scope = len(snap["scope_syms"].get(scope, []))
    ...
    members_all = _scope_members(snap, scope, rets)
    scope_avg = {}
    for k in WIN_KEYS:
...
def _series(snap: dict, scope: str, win: str, day: pd.Timestamp | None = None) -> dict:
    ...
    members_all = [m for m in _scope_members(snap, scope, rets) if m in norm.columns]
...
def api_meta():
    ...
    scopes = []
    for sc in SCOPES:
        total = len(snap["scope_syms"].get(sc["key"], []))
        have = len(_scope_members(snap, sc["key"], rets_latest))
        scopes.append({"key": sc["key"], "label": sc["label"],
                       "total": total, "with_data": have})
    close = snap["close"]
...
@app.get("/api/sectors")
def api_sectors(scope: str = "nifty50", win: str = "1M", date: str = ""):
    ...
    if day is None:
        return JSONResponse({"scope": scope, "win": win, "sectors": [], "scope_avg": {},
                             "n_total": len(snap["scope_syms"].get(scope, [])), "n_data": 0,
...
def api_constituents(scope: str = "nifty50", sector: str = "", win: str = "1M", date: str = ""):
    if scope not in SCOPE_KEYS:
        scope = "nifty50"
    if win not in WIN_DAYS:
        win = "1M"
    ...
    day = _resolve_day(snap, date or None)
    rets = _rets_asof(snap, day)
    comp, ind = snap["comp"], snap["ind"]
    members = [m for m in _scope_members(snap, scope, rets) if ind.get(m) == sector]
    rows = []
    for s in members:
        r = rets.loc[s]
        rows.append({
...
def api_chart(sym: str = "", date: str = ""):
    ...
    return JSONResponse({"sym": sym, "company": comp.get(sym, ""),
                         "sector": ind.get(sym, ""), "candles": candles})
...
<div class="wrap">
  <header>
    <div>
      <div class="title"><span class="dot"></span> Sector Analysis</div>
      <div class="sub">Sector-wise performance &amp; rotation across Nifty and Sensex constituents — equal-weight returns from the daily feed, grouped by NSE industry. Click any sector to drill into its stocks.</div>
    </div>
  ...
  <div class="card">
    <div class="resbar">
      <div class="count"><b id="secCount">0</b> sectors · <span id="scopeName">—</span> · scope avg <span id="scopeAvg">—</span></div>
...
let META=null, STATE={scope:"nifty50", win:"1M", date:""}, DATA=null, SORT={key:"win", dir:-1};
...
async function init(){
  ...
  const ss=document.getElementById("scopeSeg");
  ss.innerHTML=META.scopes.map(s=>`<button data-k="${s.key}" class="${s.key===STATE.scope?'on':''}">${s.label}<span class="c">${s.with_data}/${s.total}</span></button>`).join("");
  ss.querySelectorAll("button").forEach(b=>b.onclick=()=>{STATE.scope=b.dataset.k;syncSeg(ss,b);load();});
